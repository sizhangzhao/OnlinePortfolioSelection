...
cum_ret = 1;
lenreg = 10;
cumprod_ret = ones(n*lenreg, 1);
daily_ret = ones(n*lenreg, 1);
day_weight = ones(m, 1)/m; 

day_weight_o = zeros(m, lenreg);
daily_portfolio = zeros(n, m);

% Variables for expert
exp_ret = ones(K*lenreg, L);
exp_w = ones(K*L*lenreg, m)/m;
...
for t = 200:1:N,
    ...
    if (t >=2)
        [day_weight_all, exp_w] ...
            = cornu_kernel(data(1:t-1, :), K, L, c, exp_ret, exp_w);
    end
    
    for regs = 1:1:lenreg
        day_weight = day_weight_all(:,regs);
        % Normalize the constraint
        day_weight = day_weight./sum(day_weight);
        daily_portfolio(t, :) = day_weight';
        
        % Cal t's return and total return
        daily_ret((t-1)*lenreg + regs, 1) = (data(t, :)*day_weight)*(1-tc/2*sum(abs(day_weight-day_weight_o(:,regs))));
        cum_ret = cum_ret * daily_ret((t-1)*lenreg + regs, 1);
        cumprod_ret((t-1)*lenreg + regs, 1) = cum_ret;
        %disp(t)
        if t == N
            disp(cum_ret)
        end
        
        day_weight_o(:,regs) = day_weight.*data(t, :)'/daily_ret((t-1)*lenreg + regs, 1);
        
        % Cal t's experts return
        exp_ret((regs-1)*K + 1: regs * K, 1) = exp_ret((regs-1)*K + 1: regs * K, 1).*(data(t, :)*exp_w((regs-1) * K + 1 : regs * K, :)')';

    end

    % Debug information
    fprintf(fid, '%d\t%f\t%f\n', t, daily_ret((t-1)*lenreg + 1, 1), cum_ret);
    if (~opts.quiet_mode)
